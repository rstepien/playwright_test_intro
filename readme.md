...
- Lokator `getByTestId()` działa w oparciu o wartość atrybutu zlokalizowanego w kodzie strony data-testid.  
    ...
    ```html
    <input type="text" css="login-input" name="login" id="login_id" data-testid="login-input" class="login-input" maxlength="8" autocomplete="off" tabindex="1">
    ```
...
- Lokator `getByRole()` działa w oparciu o typy elementów i ich cechy.
    ...
    ```html
    <button type="submit" class="btn red arrow" id="execute_btn" onclick="invokeQuickTransfer()">wykonaj</button>
    ```
...
- Lokator `locator()` wykorzystuje między innymi adresy zapisane w formacie CSS.
    ...
    ```html
    <select data-classes="selector-mt-small selector-ms-small transfer_receiver" onchange="changeValues();" id="widget_1_transfer_receiver"></select>
    ```
