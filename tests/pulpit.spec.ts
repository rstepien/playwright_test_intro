...
test.describe('Pulpit Demobank tests ', () => {
    ...
    test('quick payment with correct data', async ({ page }) => {
        ...
        await page.getByTestId('password-input').fill('87iokjhn'); //87iokjhn
    ...
    test('quick payment with incorrect data', async ({ page }) => {
        ...
        await page.locator('#widget_1_transfer_receiver').selectOption('2');
        await page.locator('#widget_1_transfer_amount').fill('random_test_text');
        await page.locator('#widget_1_transfer_title').fill('pizza');
    ...
    test('successful mobile topup with select amount', async ({ page }) => {
        await page.goto('https://demo-bank.vercel.app')
        await page.getByTestId('login-input').fill('testerLO');
        await page.getByTestId('password-input').fill('87iokjhn');
        await page.getByTestId('login-button').click();
        ...
        await expect(page.locator('#show_messages')).toHaveText('Doładowanie wykonane! 102,00PLN na numer 504 xxx xxx');
      });
});